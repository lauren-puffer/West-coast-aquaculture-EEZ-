...
In order to determine the economic potential for different aquaculture projects on the West coast, I wanted to create a function that generates a visual with the total area of suitable habitat in km^2^ for different species within Exclusive Economic Zones. This required me to use both [temperature](https://coralreefwatch.noaa.gov/product/5km/index_5km_ssta.php) and [depth](https://www.gebco.net/data-products/gridded-bathymetry-data) raster data to determine species viability in each zone. The [EEZ](Flanders%20Marine%20Institute.%20(n.d.).%20EEZ%20boundaries%20%5BData%20set%5D.%20Marine%20Regions.%20https://www.marineregions.org/eez.php) shapefiles I used for this analysis pertain to the West coast of the U.S. Because EEZs are within the domain of the U.S., the states themselves can determine whether or not to establish aquaculture operations off their coastlines. This analysis sheds light on the viability of specific species to grow in the different EEZs, which in turn provides insight to their economic potential in the aquaculture industry.
...
The data for this analysis comes mostly as raster data (annual [sea surface temperature](https://coralreefwatch.noaa.gov/product/5km/index_5km_ssta.php) and [bathymetry](https://www.gebco.net/data-products/gridded-bathymetry-data)) and a single shapefile with the boundaries of [Exclusive Economic Zones](Flanders%20Marine%20Institute.%20(n.d.).%20EEZ%20boundaries%20%5BData%20set%5D.%20Marine%20Regions.%20https://www.marineregions.org/eez.php). Each of these data come from NOAA, GEBCO, and Flanders Marine Institute respectively. **Raster data** is much like a picture, as it is comprised of pixels. Each pixel in a raster has a corresponding value. The raster data I used contains the mean annual sea surface temperature in Kelvin and bathymetry of the ocean in meters. **Shape files** merely contain the shapes of different areas of interest stored as polygons. The shapfiles I used in this analysis countain the borders of the EEZs on the West Coast.
...
Fig. 2 - Raster of mean sea surface temperature in Celsius. The Western coast of the U.S. tends to be quite cold because of upwelling caused by Ekman Transport.
...
Fig. 4 - Raster of suitable habitat areas by cell. Areas indicated with a 1 are considered to be suitable habitat.
...
Fig. 5 - EEZs boundaries on the West Coast of the U.S.. EEZs are pictured in grey.

## Calculate zonal statistics

To get the total area of suitable cells in each EEZ, I calculated the zonal statistics of the cropped suitable area. To do this I used the cellSize() function to calculate the total area of each pixel in km^2^ . I then used the zonal() function to get the sum of all suitable area in km^2^ in each EEZ. This gave me all of the information I needed to make a map with the amount of suitable area contained in each EEZ.
...
oyster_suitable_area <- tm_shape(zones_clean) +
  ...
  tm_polygons(
    fill = "area_suitable",
    fill.scale = tm_scale_continuous(values = "-viridis"),  # Add this line
...
Fig. 6 - Map of suitable habitat area in km^2^ for oysters per each Exclusive Economic Zone.
...
1.  Ensures that all CRSs are the same for EEZ, sea surface temperature, and depth
2.  Subtracts 273.15 from the mean values of sea surface temperature
3.  Crops depth to the extent of sea surface temperature
4.  Creates matrix of max and min for sea surface temperature and depth
5.  Applies matrix to sea surface temperature and depth raster
6.  Multiplies sea surface temperature and depth suitable area rasters
...
Fig. 7 - Map of suitable habitat area in km^2^ for oysters per each Exclusive Economic Zone used as a test for function.
...
Fig. 8 - Map of suitable habitat area in km^2^ for California spiny lobsters per each Exclusive Economic Zone on the West Coast.
...
All of the information from this analysis can be found in the GitHub repository linked [here](https://github.com/lauren-puffer/West-coast-aquaculture-EEZ-/blob/main/README.md).
...
Neilson, D.J. and Barsky, K.C. 2011. California Department of Fish and Wildlife. Status of the Fisheries Report: California Spiny Lobster, *Panuliris interruptus*. [nrm.dfg.ca.gov/FileHandler.ashx?DocumentID=65491&inline](http://nrm.dfg.ca.gov/FileHandler.ashx?DocumentID=65491&inline).
